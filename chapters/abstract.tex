...
\begin{document}
    \begin{abstract}
        ...
        Point clouds can be employed to monitor the current state of a danger area, and CityGML models to depict its ideal state, 
        supporting rescue services in designing an incident action plan. The point cloud and the CityGML model registration is, therefore, of great importance. 

        The existing methods to register a point cloud with a CityGML model do not leverage the CityGML model’s information 
        or only work when the point cloud and the CityGML model are already coarsely registered.
